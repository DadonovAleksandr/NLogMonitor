...
- [ ] **2.4 Application Service (серверная фильтрация)**
  - [ ] Реализовать LogService.OpenFileAsync — открытие файла: чтение → парсинг → создание сессии → сохранение
  - [ ] Реализовать LogService.GetLogsAsync — **серверная** фильтрация, поиск и пагинация через LINQ
  - [ ] Построить индекс по Level для быстрой фильтрации — Dictionary<LogLevel, List<int>> (индексы записей)
  - [ ] Полнотекстовый поиск через Contains (case-insensitive) — на сервере, не на клиенте
  - [ ] Пагинация через Skip/Take — клиент получает только запрошенную страницу
...
**Definition of Done (DoD):**
- [ ] Unit tests для парсера: ≥90% покрытие NLogParser
- [ ] Парсинг файла 10MB (50K записей) < 2 сек
- [ ] Многострочные записи корректно парсятся (тест со stack trace)
- [ ] InMemorySessionStorage: тесты CRUD + TTL expiration
- [ ] LogService.GetLogsAsync возвращает отфильтрованные данные
